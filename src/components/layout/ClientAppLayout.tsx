...
export default function ClientAppLayout({ children }: { children: React.ReactNode }) {
  ...
  // 로그인한 미승인 유저만 강제 리다이렉트 (비로그인 유저는 자유 탐색 허용)
  useEffect(() => {
    if (!authLoading && user && needsVerification && !isVerifyPage && !isAuthPage) {
      router.push('/dashboard/verify');
    }
  }, [authLoading, user, needsVerification, isVerifyPage, isAuthPage, router]);
