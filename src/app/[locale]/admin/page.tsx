...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab]       = useState<'overview' | 'members' | 'txs' | 'materials' | 'supply' | 'settings' | 'marketplace' | 'simulator'>('overview');
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        {activeTab === 'simulator' && (
          <div className="max-w-5xl">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-slate-800">플랫폼 수수료 최적화 시뮬레이터</h2>
              <p className="text-sm text-slate-500 mt-1">업로드 보상, 과금, 시청 수수료율 등을 조정하여 플랫폼의 순수익과 크리에이터 수익을 시뮬레이션 해보세요.</p>
            </div>
            <FeeSimulator />
          </div>
        )}
