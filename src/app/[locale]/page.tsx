...
export default function HomePage() {
  ...
  // 영상 클릭 핸들러: 비로그인 시 로그인 모달, 로그인 시 재생
  ...
  return (
    <div className="bg-white flex flex-col h-full">
      {/* 비로그인 유저를 위한 프로모션 배너 */}
      {!user && (
        <div className="bg-gradient-to-r from-teal-600 to-teal-700 text-white px-4 sm:px-6 py-4">
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
            <div>
              <h2 className="text-lg sm:text-xl font-bold tracking-tight">{t('banner_title')} <span className="text-teal-200">RabTube</span></h2>
              <p className="text-sm text-teal-100 mt-0.5">{t('banner_desc')}</p>
            </div>
            <button
              onClick={() => router.push('/auth/login')}
              className="shrink-0 px-5 py-2.5 bg-white text-teal-700 rounded-xl font-bold text-sm hover:bg-teal-50 transition-colors shadow-lg shadow-teal-900/20"
            >
              {t('banner_btn')}
            </button>
          </div>
